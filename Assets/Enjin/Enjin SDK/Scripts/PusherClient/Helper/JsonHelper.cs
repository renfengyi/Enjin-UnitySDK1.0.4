        ...
        for (var i = 0; i < str.Length; i++)
        {
            ...
            switch (ch)
            {
                ...
                case ']':
                    ...
                    {
                        sb.AppendLine();
                        System.Linq.Enumerable.Range(0, --indent).ForEach(item => sb.Append(INDENT_STRING)); //NOTE: Including entire Linq library for a simple debug function, might not be a big deal but worth considering avoiding
                    }
                    sb.Append(ch);
                    break;
                case '"':
